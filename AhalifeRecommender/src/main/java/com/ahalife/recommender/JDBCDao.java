...
public class JDBCDao {
	...
	private static String GET_PRODUCTS_CATEGORIES = "select bcpr.CATEGORY_ID,bcpr.PRODUCT_ID from blc_product bp "
	...
	private static DataSource getDataSource() {
		if (dataSource == null) {
			...
			dataSource
					.setUrl("jdbc:mysql://sql.qa.ahalife.com:3306/broadleaf_mobile?autoReconnect=true");
	...
	public static Multimap<Long, Long> getCategoriesList() {
		if (categoryList == null) {
			...
			List<Map<String, Object>> result = getNamedParameterJdbcTemplate()
					.queryForList(GET_PRODUCTS_CATEGORIES, namedParameters);
			for (Map<String, Object> productMap : result) {

				categoryList.put((Long) productMap.get("PRODUCT_ID"),
						(Long) productMap.get("CATEGORY_ID"));
			}
		}
		return categoryList;
	}
	...
	public static List<Map<String, Object>> getProductInfo(List<Long> ids) {
		...
		List<Map<String, Object>> productInfo = getNamedParameterJdbcTemplate()
				.queryForList(GET_PRODUCT_INFO, productInfoParam);
